...
const NotificationsPage = () => {
  ...
  useEffect(() => {
    if (searchParams.get('compose') === 'support') {
      setComposeOpen(true)
    }
  }, [searchParams])
      ...
      <div className="mb-7 flex items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            type="button"
            onClick={openCompose}
            className="inline-flex items-center gap-2 rounded-xl border border-cyan-300 bg-cyan-50 px-4 py-2.5 text-base font-semibold uppercase tracking-wide text-cyan-800 hover:border-cyan-400 dark:border-cyan-400/30 dark:bg-cyan-500/10 dark:text-cyan-200"
          >
      ...
      <AnimatePresence>
        {isComposeOpen && (
          <>
            ...
            />
            <motion.aside
              className="fixed inset-x-0 bottom-0 z-50 flex h-[90vh] flex-col overflow-hidden rounded-t-3xl border border-slate-200 bg-white p-5 shadow-2xl dark:border-slate-700 dark:bg-slate-950 sm:inset-auto sm:right-4 sm:top-4 sm:bottom-4 sm:h-auto sm:w-[min(430px,calc(100vw-2rem))] sm:rounded-3xl sm:border sm:p-5"
            ...
            >
              ...
              <div className="mb-4 flex items-center justify-between gap-3 border-b border-slate-200 pb-3 dark:border-slate-800">
                <div className="min-w-0">
                  <p className="text-sm uppercase tracking-[0.22em] text-slate-500">Customer Service</p>
                  <h3 className="truncate text-2xl font-semibold text-slate-900 dark:text-slate-100">New Inquiry</h3>
                </div>
                <button
                  type="button"
                  onClick={closeCompose}
              ...
              <div className="h-[calc(90vh-8.5rem)] overflow-y-auto pr-1 sm:h-[calc(100%-4.5rem)]">
                <SupportInquiryForm compact onSubmitted={closeCompose} />
              </div>
            </motion.aside>
          </>
        )}
        ...
        {selected && (
          <>
            ...
            />
            <motion.aside
              className="fixed inset-x-0 bottom-0 z-50 h-[90vh] rounded-t-3xl border border-slate-200 bg-white p-5 shadow-2xl dark:border-slate-700 dark:bg-slate-950 sm:inset-auto sm:right-4 sm:top-4 sm:bottom-4 sm:h-auto sm:w-[min(430px,calc(100vw-2rem))] sm:rounded-3xl sm:border sm:p-5"
              initial={{ opacity: 0, x: 20, y: 28 }}
              animate={{ opacity: 1, x: 0, y: 0 }}
              exit={{ opacity: 0, x: 14, y: 24 }}
              transition={IOS_MODAL_SPRING}
            >
              ...
              <div className="flex min-h-0 flex-1 flex-col">
                <div className="min-h-0 flex-1 space-y-3 overflow-y-auto rounded-2xl bg-slate-50/70 p-2 pr-1 dark:bg-slate-900/55">
                ...
                {canReply && (
                  <div className="mt-3 rounded-2xl border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-900/90">
              ...
              <div className="mt-3 flex shrink-0 items-center justify-between border-t border-slate-200 pt-3 dark:border-slate-800">
                <button
                  type="button"
