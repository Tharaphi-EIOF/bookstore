...
const SUPPORT_GUIDELINES = [
  'Use one issue per inquiry so support can route it faster.',
  'For order issues, include your order ID.',
  'For book requests, include title, author, and ISBN if available.',
  'For legal or security concerns, include links, screenshots, or reference IDs.',
]

const SUPPORT_POLICIES = [
  {
    title: 'Response times',
    body: 'Most inquiries receive a first human reply within 24 hours.',
  },
  {
    title: 'Internal routing',
    body: 'Customer Service owns the conversation and escalates internally when another team is needed.',
  },
  {
    title: 'Urgent issues',
    body: 'Legal, security, and technical incidents should include a direct reference URL or ID for faster handling.',
  },
]
    ...
    <motion.div style={{ perspective: 1100 }}>
      <motion.div style={{ rotateX, rotateY, transformStyle: 'preserve-3d' }}>
        <div
          ...
          onMouseLeave={reset}
          onBlur={reset}
          className="relative block overflow-hidden rounded-3xl border border-cyan-200/80 bg-gradient-to-br from-white via-cyan-50/80 to-indigo-50 p-8 shadow-[0_26px_60px_-40px_rgba(14,116,144,0.35)] dark:border-cyan-400/30 dark:from-slate-900 dark:via-slate-900 dark:to-slate-950"
        >
          <motion.div
          ...
          <div className="relative">
            ...
            <h3 className="text-3xl font-black text-slate-900 dark:text-slate-100">How Support Works</h3>
            <p className="mt-2 text-base text-slate-600 dark:text-slate-300">
              Use the contact button to send one inquiry. Customer Service replies there, owns the thread, and routes your case internally when needed.
            </p>
            <ul className="mt-5 space-y-2 text-sm text-slate-600 dark:text-slate-300">
              {SUPPORT_GUIDELINES.map((item) => (
                <li key={item} className="rounded-2xl border border-white/70 bg-white/70 px-4 py-3 dark:border-white/10 dark:bg-white/5">
                  {item}
                </li>
              ))}
            </ul>
    ...
    <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      >
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/80">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Support Tools</p>
          <div className="mt-4 space-y-2">
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-950/60 dark:text-slate-300">
              Use the contact button at the bottom right when you need to start a new inquiry.
            </div>
          </div>
        </div>
      </motion.section>
      ...
      <motion.section
        ...
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ type: 'spring', stiffness: 280, damping: 30, mass: 0.82, delay: 0.05 }}
      >
        {SUPPORT_POLICIES.map((item) => (
          <article
            key={item.title}
          ...
          >
            ...
            <h3 className="mt-3 text-lg font-bold text-slate-900 dark:text-slate-100">{item.title}</h3>
