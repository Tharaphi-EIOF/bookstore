      ...
      <div className="grid min-h-screen place-items-center bg-slate-50 px-4 text-center dark:bg-slate-950">
        <div className="max-w-md rounded-3xl border border-slate-200 bg-white p-8 shadow-xl dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="mt-3 text-sm text-slate-600 dark:text-slate-300">
            Add `VITE_CLERK_PUBLISHABLE_KEY` if you want to test Clerk sign-in in this environment.
          </p>
          <Link to="/login" className="mt-5 inline-block text-sm font-semibold text-primary-600 hover:text-primary-700">
          ...
          >
            <div className="hidden lg:block">
              ...
              <h1 className="mt-6 text-5xl font-black tracking-tight text-slate-900 dark:text-slate-100">
                Test the hosted Clerk sign-in flow.
              </h1>
              <p className="mt-4 text-lg text-slate-600 dark:text-slate-300">
                Use this screen only when you want to verify Clerk behavior like social login, emails, or hosted session handling.
              </p>
            </div>
              ...
              <div className="rounded-3xl border border-slate-200/70 bg-white/80 p-8 shadow-xl shadow-slate-200/40 backdrop-blur dark:border-slate-800/80 dark:bg-slate-900/80 dark:shadow-slate-900/40">
                ...
                <div className="mt-8 overflow-hidden rounded-2xl border border-slate-200/70 bg-white dark:border-slate-800 dark:bg-slate-950/80">
                  <SignIn
                    ...
                    routing="path"
                    signUpUrl="/auth-testing/clerk/register"
                    forceRedirectUrl="/"
                    fallbackRedirectUrl="/"
                    fallback={<div className="p-6 text-sm text-slate-600 dark:text-slate-300">Loading sign-in...</div>}
                  />
                </div>
                ...
                <Link to="/login" className="mt-2 inline-block text-sm font-semibold text-slate-600 hover:text-primary-700 dark:text-slate-300">
                  Back to normal login
                </Link>
              </div>
            </div>
