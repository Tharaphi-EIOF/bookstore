...
import { usePublicStores } from '@/features/admin/services/stores'
...
const CheckoutPage = () => {
  ...
  const isEbookOnlyCart = items.length > 0 && items.every((item) => item.format === 'EBOOK')
  ...
  useEffect(() => {
    if (deliveryType !== 'HOME_DELIVERY' || !defaultAddress || selectedAddressId) return
  ...
  }, [defaultAddress, deliveryType, selectedAddressId, setValue])
  ...
  const taxRatePercent = validatePromoMutation.data?.taxRate ?? checkoutConfig?.taxRate ?? 10
  const taxableTotal = Math.max(0, subtotal - promoDiscount)
  const tax = Number((taxableTotal * (taxRatePercent / 100)).toFixed(2))
  const shipping = 0
  const total = Number((taxableTotal + tax + shipping).toFixed(2))
  ...
  const onSubmit = async (data: ShippingData) => {
    ...
    if (!isEbookOnlyCart && deliveryType === 'STORE_PICKUP' && !selectedStoreId) {
    ...
    try {
      ...
      const order = await createOrderMutation.mutateAsync({
        ...data,
        deliveryType: isEbookOnlyCart ? undefined : deliveryType,
        storeId: !isEbookOnlyCart && deliveryType === 'STORE_PICKUP' ? selectedStoreId : undefined,
        paymentProvider,
  ...
  const applySavedAddress = (addressId: string) => {
    ...
    setAddressMessage(`Loaded "${address.label}" into checkout.`)
  }

  const handleSaveCurrentAddress = async () => {
    const values = getValues()
    if (!values.fullName || !values.email || !values.phone || !values.address || !values.city || !values.state || !values.zipCode || !values.country) {
      setAddressMessage('Fill in the delivery fields before saving an address.')
      return
    }
    try {
      const saved = await createSavedAddressMutation.mutateAsync({
      ...
      setAddressMessage(`Saved "${saved.label}" to your address book.`)
    } catch {
      setAddressMessage('Failed to save the address. Please try again.')
    }
  }
                ...
                <div className="mb-5 rounded-xl border border-slate-200 p-4 dark:border-slate-700">
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                    <div className="flex-1">
                      <label className="mb-2 block text-sm font-semibold">Saved Addresses</label>
                      <select
                    ...
                    <div className="flex flex-1 gap-2">
                      <input
                        ...
                        className="h-10 flex-1 rounded-md border border-slate-300 bg-white px-3 text-sm dark:border-slate-600 dark:bg-slate-800"
                      />
                      <Button
                        type="button"
                        variant="outline"
                        onClick={handleSaveCurrentAddress}
                        isLoading={createSavedAddressMutation.isPending}
                  ...
                  {addressMessage && (
                    <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">{addressMessage}</p>
                  )}
                </div>
              )}
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow p-6 sticky top-4 dark:bg-slate-900 dark:border dark:border-slate-800">
                ...
                {items.map((item) => (
                  <div key={item.id} className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-gray-900 dark:text-slate-100">${Number(item.unitPrice ?? item.book.price).toFixed(2)} × {item.quantity}</p>
                    </div>
                    <div className="text-sm font-medium">${(Number(item.unitPrice ?? item.book.price) * item.quantity).toFixed(2)}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="border-t pt-4 space-y-2 dark:border-slate-800">
                ...
                <div className="flex justify-between text-gray-600 dark:text-slate-400"><span>Tax ({taxRatePercent.toFixed(2)}%)</span><span>${tax.toFixed(2)}</span></div>
