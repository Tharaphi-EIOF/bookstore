...
const OrdersPage = () => {
  ...
  const handleCreateReturnRequest = async (order: Order) => {
    if (!returnReason.trim()) {
      alert('Add a short reason for the return request.')
      return
    }
    try {
      ...
      alert('Failed to submit the return request.')
    }
  }
          ...
          >
            ...
            <div className="p-6">
              ...
              {(order.returnRequests?.length ?? 0) > 0 && (
                <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50/70 p-4 dark:border-slate-800 dark:bg-slate-950/30">
                  <p className="text-sm font-semibold">Return Requests</p>
                  <div className="mt-3 space-y-2">
                    {order.returnRequests?.map((request) => (
                      <div key={request.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-900">
                        <div>
                          <p className="font-medium">{request.book?.title ?? 'Order-level request'}</p>
                          <p className="text-xs text-slate-500 dark:text-slate-400">{request.reason}</p>
                        </div>
                        <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-700 dark:bg-slate-800 dark:text-slate-200">
                          {request.status}
                        </span>
                      </div>
                    ))}
                ...
                <div className="p-6 space-y-6">
                  ...
                  {(selectedOrder.status === 'CONFIRMED' || selectedOrder.status === 'COMPLETED') && (
                    <div className="rounded-xl border border-slate-200 p-4 dark:border-slate-700">
                      <h3 className="font-semibold">Return / Refund Request</h3>
                      <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                        Submit a structured request instead of opening a generic support ticket.
                      </p>
                      <div className="mt-4 grid gap-3">
                        ...
                        >
                          ...
                          {selectedOrder.orderItems.map((item) => (
                            <option key={item.id} value={item.bookId}>
                              {item.book.title}
                            </option>
                          ))}
                        </select>
                        ...
                        <textarea
                          ...
                          placeholder="Add details for support or warehouse review"
                          className="min-h-28 rounded-md border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800"
                        />
                        <div className="flex justify-end">
                          <Button
                            type="button"
                            onClick={() => handleCreateReturnRequest(selectedOrder)}
                            isLoading={createReturnRequestMutation.isPending}
                          >
                            Submit Request
                          </Button>
                        </div>
                  ...
                  {(selectedOrder.returnRequests?.length ?? 0) > 0 && (
                    <div>
                      <h3 className="font-semibold mb-3">Existing Return Requests</h3>
                      <div className="space-y-3">
                        {selectedOrder.returnRequests?.map((request) => (
                          <div key={request.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-800">
                            <div className="flex flex-wrap items-center justify-between gap-2">
                              <p className="font-medium">{request.book?.title ?? 'Order-level request'}</p>
                              <span className="rounded-full bg-white px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-700 dark:bg-slate-900 dark:text-slate-200">
                                {request.status}
                              </span>
                            </div>
