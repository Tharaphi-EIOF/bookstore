...
const INQUIRY_SECTIONS = ['overview', 'inquiries', 'templates', 'staff'] as const
const INQUIRIES_PER_PAGE = 5
type InquirySection = (typeof INQUIRY_SECTIONS)[number]
...
const AdminInquiriesPage = () => {
  ...
  const [activeSection, setActiveSection] = useState<InquirySection>('overview')
  ...
  const { data, isLoading } = useInquiries({ page: inquiriesPage, limit: INQUIRIES_PER_PAGE })
  ...
  const totalPages = Math.max(1, Math.ceil((data?.total ?? 0) / INQUIRIES_PER_PAGE))
  ...
  return (
    <div className="space-y-6">
      <section className="px-1 py-2">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{totalLabel}</p>
          </div>
      ...
      {(activeSection === 'overview' || activeSection === 'staff') && (
          ...
          {isLoading ? (
            ...
            <>
              ...
              <AdminPaginationFooter
                ...
                onPrev={() => setInquiriesPage((prev) => Math.max(1, prev - 1))}
                onNext={() => setInquiriesPage((prev) => Math.min(totalPages, prev + 1))}
