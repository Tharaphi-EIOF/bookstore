...
export const AdminOverviewContent = ({ embedded = false }: { embedded?: boolean }) => {
  ...
  return (
    <motion.div
      ...
      className={`${embedded ? 'dark:text-slate-100' : 'surface-canvas min-h-screen px-4 py-8 dark:text-slate-100 sm:px-6'}`.trim()}
    >
      <div className={`${embedded ? 'space-y-8' : 'mx-auto max-w-7xl'}`.trim()}>
        {!embedded ? (
          <div className="mb-8">
            <h1 className="text-3xl font-black tracking-tight text-gray-900 dark:text-slate-100 sm:text-4xl">Admin Dashboard</h1>
          </div>
        ) : null}
                ...
                {restockCandidates.slice(0, 5).map((book) => {
                  ...
                  return (
                    <div key={book.id} className="surface-subtle flex items-center justify-between gap-3 p-3 transition-colors hover:bg-white/95 dark:hover:bg-slate-800/75">
                      ...
                      <Link to={`/admin/purchase-requests?bookId=${encodeURIComponent(book.id)}`}>
                        <button
                          type="button"
...
const AdminPage = () => <AdminOverviewContent />
...
const dashboardCardClassName =
  'rounded-[28px] border border-white/70 bg-white/80 p-6 shadow-[0_24px_60px_-36px_rgba(15,23,42,0.28)] backdrop-blur transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_28px_70px_-34px_rgba(15,23,42,0.32)] dark:border-slate-800/80 dark:bg-slate-950/70'

const Card = ({ label, value, icon, valueClassName, href }: CardProps) => {
  const content = (
    <div className={dashboardCardClassName}>
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="text-sm font-medium uppercase tracking-[0.18em] text-slate-400 dark:text-slate-500">{label}</p>
