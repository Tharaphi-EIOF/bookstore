...
import { Book, BookOpen, ChevronRight, Clock3, Plus, RotateCcw, Trash2, X } from 'lucide-react'
import { useCreateReadingSession, useDeleteReadingSession, useReadingItems, useReadingSessions, useUpdateReadingSession } from '@/services/reading'
import BookCover from '@/components/ui/BookCover'
import ReadingActivityPanels from '@/features/library/insights/ReadingActivityPanels'
import ReadingCalendarSection from '@/features/library/insights/ReadingCalendarSection'
import ReadingInsightsSummary from '@/features/library/insights/ReadingInsightsSummary'
import type { ReadingSession } from '@/features/library/insights/types'
import { cn } from '@/lib/utils'

// Calendar, dial, and date helpers that support the reading-insights experience.
...
const MyBooksPage = () => {
  // Remote reading data and top-level UI state for the reading insights page.
  ...
  // Derived reading datasets that feed the dashboards, calendar, and composer.
  ...
  // Keep the session composer synced with duration, scroll locking, and defaults.
  useEffect(() => {
  ...
  // Composer and calendar interaction handlers for logging reading sessions.
  ...
  const handleDeleteSession = async (sessionId: string) => {
    const confirmed = window.confirm('Delete this reading session? This cannot be undone.')
    if (!confirmed) return

    await deleteReadingSession.mutateAsync(sessionId)
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 dark:bg-[#101012] dark:text-[#f5f5f1]">
      {/* Summary hero, insights panels, calendar navigation, and session composer. */}
        ...
        {isLoading || isSessionsLoading ? (
          ...
          <>
            <ReadingCalendarSection
              ...
              onHoveredDateKeyChange={setHoveredDateKey}
              periodHeading={periodHeading}
              periodHint={periodHint}
              selectedDateKey={selectedDateKey}
              setSelectedMonthAndView={(monthKey) => {
                setViewMode('MONTHLY')
              ...
              todayKey={todayKey}
              viewMode={viewMode}
              weekDayCards={weeklyCalendarDays}
              year={year}
              yearlyMonthCards={yearlyMonthCards}
            />

            <ReadingActivityPanels
              lastSevenDays={lastSevenDays}
              maxLastSevenDaysPages={maxLastSevenDaysPages}
              ...
              <div className="relative mt-4">
                          ...
                          .map((session) => (
                            ...
                            >
                              ...
                              <button
                                ...
                                className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md border border-rose-200/90 bg-rose-50 text-rose-600 transition hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-50 dark:border-rose-300/25 dark:bg-rose-500/10 dark:text-rose-200 dark:hover:bg-rose-500/20"
                ...
                {!composerError && !isComposerFutureDate && exceedsSessionLimit && (
                  ...
                  {composerSessionId && (
                    <button
                      type="button"
                      onClick={() => void handleDeleteSession(composerSessionId)}
                      disabled={deleteReadingSession.isPending}
                      className="mt-4 w-full rounded-2xl border border-rose-200 bg-rose-50 py-2.5 text-sm font-semibold text-rose-700 transition hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-50 dark:border-rose-300/25 dark:bg-rose-500/10 dark:text-rose-200 dark:hover:bg-rose-500/20"
                    >
                      {deleteReadingSession.isPending ? 'Deleting...' : 'Delete Session'}
                    </button>
                  )}
                  <button
                    type="button"
                    onClick={submitSession}
                    disabled={createReadingSession.isPending || updateReadingSession.isPending || deleteReadingSession.isPending}
